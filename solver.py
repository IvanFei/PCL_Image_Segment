...
nAveGrad = 16  # Update the weights once in 'nAveGrad' forward passes
showEvery = 50
tmp_path = 'tmp_see'
    ...
    def test(self, test_mode=0):
        ...
        for i, data_batch in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                multi_fuse = 255 * pred
                store_file = name.split("/")[0]
                if not os.path.exists(os.path.join(self.config.test_fold, name_t, store_file)):
                    os.makedirs(os.path.join(self.config.test_fold, name_t, store_file))
                print(f"[*] save path: {os.path.join(self.config.test_fold, name_t, name[:-4] + '.png')}")
    ...
    def train(self):
        ...
        for epoch in range(self.config.epoch):                          
            ...
            for i, data_batch in enumerate(self.train_loader):
                sal_image, sal_label, sal_edge, img_name = data_batch
