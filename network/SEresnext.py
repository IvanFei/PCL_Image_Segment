...
def se_resnext50_32x4d(pretrained=False, num_classes=1000):
    """
    ...
    if pretrained:
        if "se_resnext50" in cfg.MODEL.BACKBONE_CHECKPOINTS.keys():
            state_dict = torch.load(cfg.MODEL.BACKBONE_CHECKPOINTS["se_resnext50"])
        else:
...
def se_resnext101_32x4d(pretrained=False, num_classes=1000):
    """
    ...
    model = SENet(SEResNeXtBottleneck, [3, 4, 23, 3], groups=32, reduction=16,
                  ...
                  num_classes=num_classes)
    if pretrained:
        if "se_resnext101" in cfg.MODEL.BACKBONE_CHECKPOINTS.keys():
            state_dict = torch.load(cfg.MODEL.BACKBONE_CHECKPOINTS["se_resnext50"])
        else:
            settings = pretrained_settings['se_resnext50_32x4d']['imagenet']
            state_dict = model_zoo.load_url(settings["url"])
