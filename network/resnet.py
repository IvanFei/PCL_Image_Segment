...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if dilate:
            self.dilation *= stride  # Use the dilation replace of stride
            stride = 1
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        if arch not in cfg.MODEL.BACKBONE_CHECKPOINTS.keys():
            state_dict = load_state_dict_from_url(model_urls[arch],
                                                  progress=progress)
        else:
            state_dict = torch.load(cfg.MODEL.BACKBONE_CHECKPOINTS[arch])
