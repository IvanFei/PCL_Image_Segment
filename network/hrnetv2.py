...
def hrnetv2(pretrained=False, **kwargs):
    ...
    if pretrained:
        if "hrnetv2" in cfg.MODEL.BACKBONE_CHECKPOINTS.keys():
            state_dict = torch.load(cfg.MODEL.BACKBONE_CHECKPOINTS["hrnetv2"])
        else:
            state_dict = load_url(model_urls['hrnetv2'])
