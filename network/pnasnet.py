...
class PNASNet5Large(nn.Module):
    ...
    def features_forward(self, input):
        x = self.features(input)
        feature1 = x
        x = self.relu(x)
        x = self.avg_pool(x)
    ...
    def feature_and_forward(self, input):
        x = self.features(input)
        x = self.relu(x)
        x = self.avg_pool(x)
        x = x.view(x.size(0), -1)
    ...
    @property
    def features_channels(self):
        return 4320


class PNASNet5Small(nn.Module):
    def __init__(self, num_classes=1001):
        super(PNASNet5Small, self).__init__()
        self.num_classes = num_classes
        self.conv_0 = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(3, 64, kernel_size=3, stride=2, bias=False)),
            ('bn', nn.BatchNorm2d(64, eps=0.001))
        ]))
        self.cell_stem_0 = CellStem0(in_channels_left=64, out_channels_left=32,
                                     in_channels_right=64,
                                     out_channels_right=32)
        self.cell_stem_1 = Cell(in_channels_left=64, out_channels_left=64,
                                in_channels_right=160, out_channels_right=64,
                                match_prev_layer_dimensions=True,
                                is_reduction=True)
        self.cell_0 = Cell(in_channels_left=160, out_channels_left=128,
                           in_channels_right=320, out_channels_right=128,
                           match_prev_layer_dimensions=True)
        self.cell_1 = Cell(in_channels_left=320, out_channels_left=128,
                           in_channels_right=640, out_channels_right=128)
        self.cell_2 = Cell(in_channels_left=640, out_channels_left=128,
                           in_channels_right=640, out_channels_right=128)
        self.cell_3 = Cell(in_channels_left=640, out_channels_left=128,
                           in_channels_right=640, out_channels_right=128)
        self.cell_4 = Cell(in_channels_left=640, out_channels_left=256,
                           in_channels_right=640, out_channels_right=256,
                           is_reduction=True, zero_pad=True)
        self.cell_5 = Cell(in_channels_left=640, out_channels_left=256,
                           in_channels_right=1280, out_channels_right=256,
                           match_prev_layer_dimensions=True)
        self.cell_6 = Cell(in_channels_left=1280, out_channels_left=256,
                           in_channels_right=1280, out_channels_right=256)
        self.cell_7 = Cell(in_channels_left=1280, out_channels_left=256,
                           in_channels_right=1280, out_channels_right=256)
        self.cell_8 = Cell(in_channels_left=1280, out_channels_left=512,
                           in_channels_right=1280, out_channels_right=512,
                           is_reduction=True)
        self.cell_9 = Cell(in_channels_left=1280, out_channels_left=512,
                           in_channels_right=2560, out_channels_right=512,
                           match_prev_layer_dimensions=True)
        self.cell_10 = Cell(in_channels_left=2560, out_channels_left=512,
                            in_channels_right=2560, out_channels_right=512)
        self.cell_11 = Cell(in_channels_left=2560, out_channels_left=512,
                            in_channels_right=2560, out_channels_right=512)
        self.relu = nn.ReLU()
        self.avg_pool = nn.AvgPool2d(11, stride=1, padding=0)
        self.dropout = nn.Dropout(0.5)
        self.last_linear = nn.Linear(2560, num_classes)
...
def pnasnet5large(num_classes=1001, pretrained='imagenet'):
    ...
    if pretrained:
        ...
        if "pnasnet" in cfg.weights_path.keys():
            model.load_state_dict(torch.load(cfg.weights_path["pnasnet"]))
        else:
            model.load_state_dict(model_zoo.load_url(settings['url']))
...
def pnasnet5small(num_classes=1001, pretrained="imagenet"):
    if pretrained is True:
        pretrained = "imagenet"
    if pretrained is False:
        pretrained = None
    ...
    if pretrained:
        model = PNASNet5Small(num_classes=num_classes)
        if "pnasnet5small" in cfg.weights_path.keys():
            model.load_state_dict(torch.load(cfg.weights_path["pnasnet5small"]))
        else:
            raise FileNotFoundError("The PNASNet5Small is not found in url.")
