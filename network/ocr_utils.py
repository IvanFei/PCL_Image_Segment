...
from network.extra_modules import BNReLU, get_aspp
...
class SpatialGather_Module(nn.Module):
    ...
    def forward(self, feats, probs):
        ...
        probs = probs.view(batch_size, c, -1)  # batch x num_classes x hw
        feats = feats.view(batch_size, feats.size(1), -1)

        feats = feats.permute(0, 2, 1).contiguous()  # batch x hw x num_feats
        probs = F.softmax(self.scale * probs, dim=2)
        ocr_context = torch.matmul(probs, feats)  # batch x num_classes x num_feats
...
class ObjectAttentionBlock(nn.Module):
    ...
    def __init__(self, in_channels, key_channels, scale=1):
        ...
        self.f_pixel = nn.Sequential(
            ...
            BNReLU(key_channels),
            nn.Conv2d(in_channels=self.key_channels, out_channels=self.key_channels, kernel_size=1,
                      stride=1, padding=0, bias=False),
    ...
    def forward(self, x, proxy):
        ...
        query = self.f_pixel(x).view(batch_size, self.key_channels, -1)  # batch x key_c x hw
        query = query.permute(0, 2, 1).contiguous()  # batch x hw x key_c
        key = self.f_object(proxy).view(batch_size, self.key_channels, -1)  # batch x key_c x num_classes
        value = self.f_down(proxy).view(batch_size, self.key_channels, -1)  # batch x key_c x num_classes
        value = value.permute(0, 2, 1).contiguous()  # batch x num_classes x key_c
    ...
    def __init__(self, in_channels, key_channels, out_channels, scale=1, dropout=0.1):
        ...
        if self.OCR_ASPP:
            self.aspp, aspp_out_ch = get_aspp(
                in_channels, bottleneck_ch=self.ASPP_BOT_CH, output_stride=8
            )
