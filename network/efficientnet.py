...
from network.efficientnet_utils import url_map, url_map_advprop, get_model_params
...
class EfficientNetEncoder(EfficientNet, EncoderMixin):
    ...
    def get_stages(self):
        return [
            nn.Identity(),
            nn.Sequential(self._conv_stem, self._bn0, self._swith),
            self._blocks[:self._stage_idx[0]],
            self._blocks[self._stage_idx[0]:self._stage_idx[1]],
            self._blocks[self._stage_idx[1]:self._stage_idx[2]],
            self._blocks[self._stage_idx[2]:]
        ]
